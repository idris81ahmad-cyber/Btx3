    ...
    <div className="max-w-7xl mx-auto px-6 py-10">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-[#6B5F54] mt-2"> {filteredProducts.length} beautiful pieces • From Kano with love</p>
        </div>
              ...
              <div className="space-y-1.5 text-sm">
                {categories.map((cat) => (
                  ...
                  >
                    {cat}
                    {cat !== "All Categories" && (
                      <span className="text-xs opacity-60">{products.filter(p => p.category === cat).length}</span>
                    )}
                  </button>
                ))}
              </div>
