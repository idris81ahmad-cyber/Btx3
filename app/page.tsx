    ...
    <div className="overflow-hidden">
      ...
      <section className="relative h-[92vh] flex items-center justify-center bg-[#2C2522] text-white">
        ...
        <div className="relative z-10 max-w-5xl px-6 text-center">
          ...
          <div className="mt-12 text-xs tracking-[2px] text-white/50">SCROLL TO DISCOVER</div>
        </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
            {categories.map((cat, idx) => (
              <Link 
                key={idx} 
                href={cat.href}
                className="group border border-[#D4C9B8] hover:border-[#6B2D3C] rounded-3xl p-6 flex flex-col justify-between h-full transition-all hover:shadow-sm bg-[#F8F4EC]"
              >
                <div>
                  <div className="font-semibold text-xl tracking-tight group-hover:text-[#6B2D3C] transition">{cat.name}</div>
                  <div className="text-sm text-[#6B5F54] mt-1">{cat.count} premium pieces</div>
                </div>
                <div className="mt-8 text-xs text-[#C5A46E] flex items-center gap-1 group-hover:gap-2 transition-all">
